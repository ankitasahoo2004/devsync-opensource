...
router.get('/user/:username', githubController.getUserData);
...
router.get('/contributions/:username', githubController.getContributions);
...
router.get('/prs/update', githubController.updatePrStatus);
// router.get('/prs/update', async (req, res) => {
//     try {
...
router.get('/events/:username/pushes', async (req, res) => {
    try {
        console.log(`Fetching push events for user: ${req.params.username}`);
...
router.post('/graphql', isAuthenticated, isAdmin, githubController.proxyGraphQL);
