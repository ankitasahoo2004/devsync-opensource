...
const { checkBadges, getCurrentBadge } = require("../utils/badgeAssigner");
...
router.post("/projects/:projectId/review", async (req, res) => {
  ...
  try {
    ...
    if (projectOwner) {
      if (status === "accepted") {
        await emailService.sendProjectAcceptedEmail(
          projectOwner.email,
          project
        );
...
router.patch("/projects/:projectId/points", async (req, res) => {
  if (!req.isAuthenticated()) {
    return res.status(401).json({ error: "Unauthorized" });
  }
...
router.get("/pending-prs", async (req, res) => {
  if (!req.isAuthenticated()) {
    return res.status(401).json({ error: "Unauthorized" });
  }
        ...
        try {
          ...
          const enrichedPR = {
            ...
            user: {
              avatar_url: user
                ? user.avatarUrl
                : `https://github.com/${pr.username}.png`,
              login: pr.username,
          ...
          console.error(
            `Error enriching PR data for ${pr.username}:`,
            userError
          );
...
router.post("/pr/:prId/approve", async (req, res) => {
  if (!req.isAuthenticated()) {
    return res.status(401).json({ error: "Unauthorized" });
  }
...
router.post("/pr/:prId/reject", async (req, res) => {
  if (!req.isAuthenticated()) {
    return res.status(401).json({ error: "Unauthorized" });
  }
...
router.patch("/pr/:prId/points", async (req, res) => {
  if (!req.isAuthenticated()) {
    return res.status(401).json({ error: "Unauthorized" });
  }
...
router.get("/rejected-prs", async (req, res) => {
  if (!req.isAuthenticated()) {
    return res.status(401).json({ error: "Unauthorized" });
  }
  ...
  try {
    const rejectedPRs = await PendingPR.find({ status: "rejected" }).sort({
      reviewedAt: -1,
    });

    const enrichedPRs = await Promise.all(
      rejectedPRs.map(async (pr) => {
...
router.delete("/pr/:prId", async (req, res) => {
  if (!req.isAuthenticated()) {
    return res.status(401).json({ error: "Unauthorized" });
  }
...
router.get("/all-prs", async (req, res) => {
  if (!req.isAuthenticated()) {
    return res.status(401).json({ error: "Unauthorized" });
  }
  ...
  try {
    const allPRs = await PendingPR.find({}).sort({ submittedAt: -1 }).lean();
...
router.get("/users", async (req, res) => {
  ...
  try {
      ...
      users.map(async (user) => {
        try {
          ...
          const pendingPRsCount = await PendingPR.countDocuments({
            $or: [{ userId: user.githubId }, { username: user.username }],
            status: "pending",
          });
          ...
          const approvedPRs = await PendingPR.find({
            ...
            .select("suggestedPoints")
            .lean();

          const calculatedPoints = approvedPRs.reduce(
            (sum, pr) => sum + (pr.suggestedPoints || 50),
            0
          );
          ...
          return {
            ...
            rejectedPRsCount,
            totalPRsSubmitted:
              pendingPRsCount + approvedPRsCount + rejectedPRsCount,
          ...
          console.error(
            `Error enriching user data for ${user.username}:`,
            userError
          );
    ...
    const summary = {
      ...
      totalPRsSubmitted: enrichedUsers.reduce(
        (sum, u) => sum + u.totalPRsSubmitted,
        0
      ),
    };
...
router.post("/users/:userId/reset-auth", async (req, res) => {
  if (!req.isAuthenticated()) {
    return res.status(401).json({ error: "Unauthorized" });
  }
...
router.post("/users/:userId/resync", async (req, res) => {
  if (!req.isAuthenticated()) {
    return res.status(401).json({ error: "Unauthorized" });
  }
