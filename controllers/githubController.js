...
const getContributions = async (req, res) => {
  console.log("Fetching contributions for:", req.params.username);
  try {
...
const updatePrStatus = async (req, res) => {
  try {
          ...
          try {
            ...
            for (const pr of prData.items) {
              try {
                ...
                console.error(
                  `Error processing PR ${pr.number} for ${user.username}:`,
                  prError.message
                );
                continue;
              }
            ...
            const updatedPoints = await calculatePointsFromApprovedPRs(
              user.githubId
            );
            const updatedBadges = await checkBadges(
              approvedMergedPRs,
              updatedPoints
            );
    ...
    const summary = {
      ...
      totalNewSubmissions: results.reduce(
        (sum, r) => sum + (r.newSubmissions || 0),
        0
      ),
...
const proxyGraphQL = async (req, res) => {
  ...
  try {
    console.log("Proxying GraphQL request for user:", req.user.username);
